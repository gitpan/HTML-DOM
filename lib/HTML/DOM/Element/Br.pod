...
=head1 SYNOPSIS
  ...
  $elem->clear(1); # set attribute
  $elem->clear;    # get attribute
  $elem->tagName;
  # etc

=head1 DESCRIPTION

This class implements 'br' elements in an HTML::DOM tree. 
It 
implements the HTMLBRElement DOM interface and inherits from 
L<HTML::DOM::Element> (q.v.).

=head1 METHODS

The only method that this class implements itself, and does not inherit, is
C<clear>, which returns the corresponding HTML attribute. If you pass an
