...
=head1 SYNOPSIS
  ...
  $elem->width(1); # set attribute
  $elem->width;    # get attribute
  $elem->tagName;
  # etc

=head1 DESCRIPTION

This class implements 'pre' elements in an HTML::DOM tree. 
It 
implements the HTMLPreElement DOM interface and inherits from 
L<HTML::DOM::Element> (q.v.).

=head1 METHODS

The only method that this class implements itself, and does not inherit, is
C<width>, which returns the corresponding HTML attribute. If you pass an
